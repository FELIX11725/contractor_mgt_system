        ...
        <section class="container px-4 mx-auto">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="window.location.href='{{ route('addproject') }}'" class="btn bg-gray-900 text-white">
                        Add New
                    </button>
                                ...
                                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                                    <div class="border-b border-gray-200 pb-2 mb-4 px-4 pt-4">
                                        <div class="flex items-center justify-between">
                                            <h3 class="text-lg font-medium text-gray-900" id="modal-headline">
                                                @if($modalType === 'edit') Edit Project
                                                @elseif($modalType === 'view') View Project 
                                                @else Add Project 
                                                @endif
                                            </h3>
                                    ...
                                    <div class="px-4 pb-4">  {{-- Container for view mode --}}
                                        <div class="mt-2">
                                            ...
                                            <span id="project_name" class="block text-sm text-gray-900">{{ $project_name }}</span>
                                        </div>
                                        <div class="mt-2">
                                            <label for="location" class="block text-sm font-medium text-gray-700">Project Location:</label>
                                            <span id="location" class="block text-sm text-gray-900">{{ $location }}</span>
                                        </div>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <div id="project_description" class="block text-sm text-gray-900 trix-content">{!! $project_description !!}</div> </div>
                                            <div class="mt-2">
                                            <label for="start_date" class="block text-sm font-medium text-gray-700">Project Start Date:</label>
                                            <span id="start_date" class="block text-sm text-gray-900">{{\Carbon\Carbon::parse($start_date )->format('M j, Y') }}</span>                        
                                        </div>
                                            <div class="mt-2">
                                                <label for="end_date" class="block text-sm font-medium text-gray-700">Estimated Project End Date:</label>
                                                <span id="end_date" class="block text-sm text-gray-900">{{\Carbon\Carbon::parse($end_date )->format('M j, Y') }}</span>                        
                                            </div>
                                   ...
                                   <form wire:submit.prevent="{{ $modalType === 'edit' ? 'update' : 'save' }}" class="px-4 pb-4">
                                        ...
                                        @csrf
                                        <div class="mt-2">
                                            <label for="project_name" class="block text-sm font-medium text-gray-700">Project Name <span class="inline-block h-5 w-5 text-red-500">*</span></label>
                                        <input type="text" wire:model="project_name" id="project_name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                        <x-input-error for="project_name" />
                                        </div>
                                        <div class="mt-2">
                                            <label for="location" class="block text-sm font-medium text-gray-700">Project location <span class="inline-block h-5 w-5 text-red-500">*</span></label>
                                            <input type="text" wire:model="location" id="location" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-800">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Location</th>
                                        <th scope="col" class="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Project type</th>
                                        ...
                                        <th scope="col" class="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Proposed end date</th>
                                        ...
                                        <tr>
                                            <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                                <div class="inline-flex items-center gap-x-3">
                                                    <input type="checkbox" wire:model="selectedContractors" value="{{ $project->id }}" class="text-blue-500 border-gray-300 rounded dark:bg-gray-900 dark:ring-offset-gray-900 dark:border-gray-700">
                                                    <div class="flex items-center gap-x-2">
                                                        ...
                                                        <div>
                                                            <h2 class="font-medium text-gray-800 dark:text-white">{{ $project->project_name }} </h2>
                                                            <p class="text-sm font-normal text-gray-600 dark:text-gray-400"></p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{{ $project->location }}</td>
                                            <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{{ $project->project_type->name }}</td>
                                            <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap"> {{ \Carbon\Carbon::parse($project->start_date )->format('M j, Y') }}</td>
                                            <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap"> {{ \Carbon\Carbon::parse($project->end_date )->format('M j, Y') }}</td>
                                            <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap"> <x-status :status="$project->project_status" /></td>
                                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                <div class="flex items-center gap-x-6">
                                                    <button wire:click="openModal('view',{{ $project->id }})" class="text-gray-500 transition-colors duration-200 dark:hover:text-green-500 dark:text-gray-300 hover:text-green-500 focus:outline-none">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                        </svg>
                                                    </button>
                                                    <button wire:click="openModal('edit',{{ $project->id }})" class="text-gray-500 transition-colors duration-200 dark:hover:text-yellow-500 dark:text-gray-300 hover:text-yellow-500 focus:outline-none">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
>
    ...
    >
        ...
        <div class="mt-6 flex justify-end space-x-4">
            ...
            <button 
                wire:click="delete(projectIdToDelete)"
                @click="deleteModalOpen = false"
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="items-center hidden lg:flex gap-x-3">
                    @for ($i = 1; $i <= $projects->lastPage(); $i++)
                        <a href="#" wire:click="setPage({{ $i }})" class="px-2 py-1 text-sm {{ $i === $projects->currentPage() ? 'text-blue-500 bg-blue-100/60' : 'text-gray-500' }} rounded-md hover:bg-gray-100">{{ $i }}</a>
                    @endfor
                </div>
                <a href="#" class="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800">
                    <span>Next</span>
